...
import fontData2 from './data/Balsamiq Sans_Regular.json' assert {type: 'json'};
...
let video6 , videoImage6 ,videoImageContext6, videoTexture6;
...
class BasicCharacterController {
  ...
  _LoadModels() {
    ...
    let fbxPath;
        ...
        if(characters[0] == 2) {
          characterFolder = './resources/claire/';
          fbxPath = 'claire.fbx';
  ...
  EnterWall7Body.addEventListener("collide", () => {
    console.log("미궁입구");
      ...
      if(characters[0] == 1) { //남자캐릭터
        ...
        window.location.href = 'http://digitalgamenomad.cf:5501/?image=1';
      }else if(characters[0]= 2){//여자캐릭터 
        //window.location.href = 'http://127.0.0.1:5501/?image=2';
        window.location.href = 'http://digitalgamenomad.cf:5501/?image=2';
      }
...
scene.add(movieScreen);
 ...
 ///////////
  ...
  //영상소스 주소 
    video6.src = "videos/Network.mp4";
   
    video6.load(); // must call after setting/changing source
    video6.muted = true;
    videoImage6 = document.createElement( 'canvas' );
    //영상의 사이즈 
    videoImage6.width = 2560;
    videoImage6.height = 1440;
    ...
    var movieMaterial6 = new THREE.MeshBasicMaterial( { map: videoTexture6, overdraw: true, side:THREE.DoubleSide } );

    //영상창 크기 설정 (창크기에 맞춰 영상재생됨)
    var movieGeometry6 = new THREE.PlaneGeometry(80, 20);
    var movieScreen6 = new THREE.Mesh( movieGeometry6, movieMaterial6 );
    movieScreen6.position.set(-74.35, 10, 0);
    movieScreen6.rotateY(Math.PI/2);
  
    scene.add(movieScreen6);

    var movieScreen7 = new THREE.Mesh( movieGeometry6, movieMaterial6 );
    movieScreen7.position.set(74.35, 10, 0);
    movieScreen7.rotateY(Math.PI/2);
  
    scene.add(movieScreen7);
...
//////////
  ...
  const medical_tent1 = new GLTFLoader(manager);
  medical_tent1.load('./resources/medical_tent/scene.gltf', (gltf) => {

    const model = gltf.scene;
    model.position.set(-42, 0, 65);
    model.rotation.set(0, Math.PI * 1.7, 0);
    model.scale.set(3, 3, 3);
    scene.add(model);
  });

  const medical_tent2 = new GLTFLoader(manager);
  medical_tent2.load('./resources/medical_tent/scene.gltf', (gltf) => {

    const model = gltf.scene;
    model.position.set(-33, 0, 60);
    model.rotation.set(0, Math.PI * 1.7, 0);
    model.scale.set(3, 3, 3);
    scene.add(model);
  });
      ...
      if (sphere.type === 'Group') {
        sphere.children.forEach(child => {
          ...
          if (intersects.length > 0 && intersects[0].object && intersects[0].object.userData.className === 'tunche') {
            ...
            window.open('http://digitalgamenomad.cf:5501/?image=1', '_blank')
          }else if (intersects.length > 0 && intersects[0].object && intersects[0].object.userData.className === 'game'&& characters[0] == 2)  {
            window.open('http://digitalgamenomad.cf:5501/?image=2', '_blank')
          }
        })
      }
